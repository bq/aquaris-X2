...
{
	seq_printf(file,"[Vendor]GF3258DN1,Holitech\n");
	return 0;
}

static int proc_open(struct inode *inode,struct file *file)
{
	printk("gf3258DN1 proc_open\n");
...
#endif
{
	...
	printk("[Goodix] Begin goodix fingerprint probe!\n");
	...
	if (gf_parse_dts(gf_dev))
		goto error_hw;
	udelay(1000);
	if(!(status_id = gpio_get_value(14))){
		printk("[Goodix] Failed to probe goodix fingerprint for no goodix_module!\n");
		gf_cleanup(gf_dev);
		return 0;
	}
	...
	gf_disable_irq(gf_dev);
	 ...
	 if (NULL == proc_entry)
         {
             printk(" gf3258DN1 Couldn't create proc entry!");
             return -ENOMEM;
         }
         else
         {
             printk("gf3258DN1 Create proc entry success!");
         }

	pr_info("version V%d.%d.%02d\n", VER_MAJOR, VER_MINOR, PATCH_LEVEL);
	printk("[Goodix] Probe goodix fingerprint success!\n");
...
static int __init gf_init(void)
{
	 ...
	 */
	if(fpsensor != 2){
		pr_err("andy gf_init failed as fpsensor=%d(2=gx)\n",fpsensor);
		return -1;
	}
	BUILD_BUG_ON(N_SPI_MINORS > 256);
	printk("[Goodix] Begin goodix fingerprint init!\n");
