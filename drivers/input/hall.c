...
static int hall_power_on(struct device *pdev)
{
	...
	if (IS_ERR(hall_vio)) {
		ret = -1;
		pr_err("hall regulator get failed vio ret=\n");
		return ret;
	}
	...
	if (regulator_count_voltages(hall_vio) > 0) {
		...
		if (ret) {
			pr_err("hall regulator set_vtg failed vio ret=\n");
			goto reg_vio_put;
		}
	}
	...
	if (ret) {
		pr_err("hall regulator vio enable failed ret=\n");
		return ret;
	}
		...
		struct class_attribute *attr, char *buf)
{
	...
	pr_err("hall: hall_state_show state = %d, custome_state=%d\n", global_hall_info->hall_switch_state, state);
...
static int hall_probe(struct platform_device *pdev)
{
	...
	if (gpio_is_valid(hall_info->irq_gpio)) {
		...
		pr_err("hall irq gpio not provided\n");
		goto free_input_device;
	}
