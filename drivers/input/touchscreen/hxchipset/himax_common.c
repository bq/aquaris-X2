...
//		return;
	...
	snprintf(info, sizeof(info), "[fw]0x%x,[tv]0x%04x,[ic]HX83112A", ic_data->vendor_fw_ver,ic_data->vendor_config_ver); 
	snprintf(dev_info, sizeof(dev_info), "[Vendor]%s,[fw]0x%x,[ic]HX83112A", module, ic_data->vendor_fw_ver);
	if(initialized){
		update_tp_fm_info(info);
		I("%s: update tp_info:%x,%04x", __func__,ic_data->vendor_fw_ver,ic_data->vendor_config_ver);
	}
	else {
		init_tp_fm_info(0, info, module);
		I("%s: initialized tp_info:%x,%04x", __func__,ic_data->vendor_fw_ver,ic_data->vendor_config_ver);
        initialized = true;
   }
...
static int i_update_FW(void)
{
	...
	if((lcd_panel_id == 0)&&(tpsensor == 2)){
		...
		I("%s: enter new DJN fw tpsensor= %d\n", __func__,tpsensor);
	}
