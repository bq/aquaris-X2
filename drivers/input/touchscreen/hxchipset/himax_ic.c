...
#else
    if(lcd_panel_id ==0)
		ic_data->HX_RX_NUM		= DJN_FIX_HX_RX_NUM;
	else
		ic_data->HX_RX_NUM      	= XINLI_FIX_HX_RX_NUM;
...
void himax_read_FW_ver(struct i2c_client *client)
{
	...
	while(reload_status == 0)
	{
		...
		cmd[0] = 0xc0;
		himax_register_read(client, cmd, 4, data, false);
		//himax_sensor_on(client,1);
		I("data[3]:%d,data[2]:%d,data[1]:%d,data[0]:%d\n",data[3],data[2],data[1],data[0]);

		if(data[3]==0x00 && data[2]==0x00 && data[1]==0x72 && data[0]==0xc0)
		{
			I("reload OK! \n");
			reload_status = 1;
			break;
...
if((lcd_panel_id == 0)&&(tpsensor == 2)){
	I("%s: DJN OLD PANEL lcd_panel_id = %d\n", __func__,lcd_panel_id);
#ifdef HX_AUTO_UPDATE_FW
	...
	I("%s: enter new panel tpsensor = %d, lcd_panel_id = %d\n", __func__,tpsensor,lcd_panel_id);
#ifdef HX_AUTO_UPDATE_FW
